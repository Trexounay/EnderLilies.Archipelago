...
# creates a region for each room (Center01)
# creates a connection for each entrance (Center01LeftDoor) to a region of the same name as the entrance (Center01LeftDoor)
**{
    region : RegionData(region, [ExitData(src, src) for (src, dst) in connections])
    for region, connections in room_connections.items()
},
# creates a region for each entrance (Center01LeftDoor)
# creates a connection to the other side (Center05CenterDoor) with the name "Center01LeftDoor to Center05CenterDoor"
**{
...
# we end up with:
# Center01 -(Center01LeftDoor)-> Center01LeftDoor -(Center01LeftDoor to Center05CenterDoor)-> Center05CenterDoor -(no name)-> Center05
# Center05 -(Center05CenterDoor)-> Center05CenterDoor -(Center05CenterDoor to Center01LeftDoor)-> Center01LeftDoor -(no name)-> Center01
