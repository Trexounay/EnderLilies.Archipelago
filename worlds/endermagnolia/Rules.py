...
macros = {
    'CANBREAK' : lambda s, p : s.has("Lito", p) or s.has("Nola", p),
}
...
def get_locations_rules(p: int) -> Dict[str, CollectionRule]:
    return {
        "Goal": lambda s : s.can_reach_location("Ruins 13 - Homunculus Research Log 1", p),
        "Ruins 5 - Charmed Fragment": lambda s : s.has("Hati's Charge", p) and s.has("Aerial Jump", p),
    }
